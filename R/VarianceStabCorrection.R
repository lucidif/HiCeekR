matrixToVec<-function(m, seg, chrs)
{
	...
	for (i in 1:nchrs)
	{
		...
		cis = c(cis, gdata::upperTriangle(m[i1:iN, i1:iN]))
		if (iN < iTot)
		{
...
correctPairCIM<-function(m, isCis)
{
	...
	if (isCis)
	{
		...
		cis = gdata::upperTriangle(m, diag = F)
		origSize = length(cis)
		cis = padding(cis)
		correctedCis = correctData(cis)


		gdata::upperTriangle(mCorrected) = correctedCis[1:origSize]
		ind <- lower.tri(mCorrected)
		mCorrected[ind] <- t(mCorrected)[ind]
		diag(mCorrected) = correctedSelf[1:length(diag(m))]
