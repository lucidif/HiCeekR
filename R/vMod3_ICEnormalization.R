        ...
        shiny::fluidRow( #lateral panel
            ...
            shiny::wellPanel(
                ...
                #shiny::fluidRow( #parameters row
                    ...
                    shiny::fluidRow( #2,
                        shiny::column( 8,
                                 shiny::helpText("set NA to min:   ")
                        )
                                  ),
                    shiny::fluidRow( #2,
                        shiny::column(8,
                                  shiny::checkboxInput(diffHicNormNs('NAmin'),
                                                       label="",
                                                       value=FALSE)
                        )
                                  ),
...
diffHic_Normalization_Server <- function(input, output, session,
    ...
    observeEvent (input$rdButton ,{
        ...
        if (input$chrInt1==input$chrInt2){
            #intsett<<-saveMe$intSet
            #chr<<-input$chrInt1
            #corr<<-saveMe$correction$truth
            #natt<<-input$NAmin
            #wktt<<-pointin(wdPath,'Normalization',sys=TRUE)
            saveMe$NormMatrix<- as.brute.matrix (saveMe$intSet, first=input$chrInt1 ,
                                                 second=input$chrInt2 ,
                                                 fill=saveMe$correction$truth,
                                                 alternativeFill=TRUE,
                                                 NAmin=input$NAmin,
                                                 wkFolder=pointin(wdPath,'Normalization',sys=TRUE)
            )
        }else{
            normA<-as.brute.matrix (saveMe$intSet, first=input$chrInt1 ,
                                    second=input$chrInt1 ,
                                    fill=saveMe$correction$truth,
                                    alternativeFill=TRUE,
                                    NAmin=input$NAmin,
                                    wkFolder=pointin(wdPath,'Normalization',sys=TRUE))
            normB<-as.brute.matrix (saveMe$intSet, first=input$chrInt2 ,
                                    second=input$chrInt2 ,
                                    fill=saveMe$correction$truth,
                                    alternativeFill=TRUE,
                                    NAmin=input$NAmin,
                                    wkFolder=pointin(wdPath,'Normalization',sys=TRUE))
            normAB<-as.brute.matrix (saveMe$intSet, first=input$chrInt1 ,
                                     second=input$chrInt2 ,
                                     fill=saveMe$correction$truth,
                                     alternativeFill=TRUE,
                                     NAmin=input$NAmin,
                                     wkFolder=pointin(wdPath,'Normalization',sys=TRUE))
            saveMe$NormMatrix<-as.sym.matrix(chrAma=normA,chrBma = normB,chrABma=normAB)
            rm(normA)
            rm(normB)
            rm(normAB)
        }

        print ('1 NormMatrix.....OK')
        #print (rownames(NormMatrix))
        #View (NormMatrix)

        #make reports
        len<-length(colnames(saveMe$NormMatrix))
        lsnames<-list(rownames(saveMe$NormMatrix),colnames(saveMe$NormMatrix))
        saveMe$NormMatrix<-matrix(mapply(saveMe$NormMatrix, FUN=as.numeric),ncol=len,nrow=len,dimnames=lsnames)

        sparsity<-(length(which(saveMe$NormMatrix>0)))/(dim(saveMe$NormMatrix)[1]*dim(saveMe$NormMatrix)[2])

        #output$histSlot<-shiny::renderImage({
            #outfile <- tempfile(fileext = '.png')
            #png(outfile, width = 400, height = 300)
            #hist(log2(saveMe$NormMatrix),col="cadetblue1",  main="values distribution")
            #dev.off()
            #hist(log2(saveMe$NormMatrix),col="cadetblue1",  main="values distribution")
        ...
        output$resultSlot<-shiny::renderUI({
            shiny::fluidRow(
                shiny::fluidRow(
                    ...
                    ,shiny::column(3,shiny::helpText(input$chrInt1))
                    ,shiny::column(2,shiny::helpText("VS"))
                    ,shiny::column(3,shiny::helpText(input$chrInt2))
                ),
                shiny::fluidRow(
    ...
    observeEvent (input$expBut,{
        ...
        toSaveMatrix2<-saveMe$NormMatrix
        ...
        if (input$chrRange=='VS'){
            ...
            print (c('matrix saved as:', rawfilename))
            ...
            #HCRwrite (toSaveMatrix2, file=rawfilename, path=pointin(wdPath,'Normalization'), row.names=TRUE)
            HCRwrite (toSaveMa, file=rawfilename, path=pointin(wdPath,'Normalization'), row.names=TRUE)
    ...
    observeEvent (input$fromMatrixStartBut,{
        ...
        saveMe$correction <- correctedContact (saveMe$intSet,
                                               ignore.low=input$igLow,
                                               winsor.high=input$winHi)
        print ('correction.....OK')
    ...
    observeEvent (input$startBut,{
        ...
        saveMe$correction <- correctedContact (saveMe$intSet, ignore.low=0, winsor.high=0)
        print ('correction.....OK')
        ##creare la normmatrix inchiomma la ram devo capire perche???
        ##NormMatrix<<- as.matrix (red, first='chr1', second='chr1', fill=correction$truth)
        print (c('chromosomo of interest',input$chromo))
        ...
        output$resultSlot<-shiny::renderUI({
            shiny::fluidRow(
                ...
                shiny::fluidRow(
                    #shiny::column(4,shiny::helpText("file name")),
                    shiny::column(12,shiny::imageOutput(diffHicNormNs("histSlot")))
                )
